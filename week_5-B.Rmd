...
draws <- simulate_election(100) %>%
  pivot_longer(names_to = "state",
               values_to = "trump_share",
                cols = 1:51) %>%
  filter(state == "Arizona" | state == "Georgia")
draws
```
...
ggplot(draws, aes(trump_share, fill = state)) + 
  geom_density() +
  theme_classic() +
  facet_wrap(~state)
...
q3_elections %>%
  group_by(oh_win == TRUE) %>%
  summarize(pa_avg = mean(pa_win))
  
# Win PA given OH = 35%, win PA given lose OH = 6.9%
```
...
elections_100 %>%
mutate(df_pivoted =
 map(simulations, ~pivot_longer(., 
                                cols = 1,
                                names_to = "states")))
View(elections_100$df_pivoted)
